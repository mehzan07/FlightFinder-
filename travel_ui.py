...
def travel_ui():
    if request.method == "POST":
        ...
        if trip_type == "one-way":
            user_input = (
                f"Fly one-way from {origin} to {destination} on {date_from_raw} "
                f"for {passengers} passengers in {cabin_class} class"
            )
        else:
            user_input = (
                f"Fly from {origin} to {destination} from {date_from_raw} to {date_to_raw} "
                f"for {passengers} passengers in {cabin_class} class"
            )
        ...
        for prepared_flight in flights:
            prepared_flight["origin"] = trip_info.get("origin", "Unknown origin")
            prepared_flight["destination"] = trip_info.get("destination", "Unknown destination")
...
@travel_bp.route("/offer/<int:offer_id>")
def view_offer(offer_id):
    offer = offers_db.get(offer_id)
    #print("DEBUG:", offer.depart)  # Add this to verify it's not None
